...
public class AuthorityRuleController {
    ...
    public Result<List<AuthorityRuleEntity>> apiQueryAllRulesForMachine(@RequestParam String app,
        ...
        try {
            List<AuthorityRuleEntity> rules =
                    this.getNacosRules(app, ip, port);
    ...
    public Result<AuthorityRuleEntity> apiAddAuthorityRule(@RequestBody AuthorityRuleEntity entity) {
        ...
        if (!publishRulesToNacos(entity.getApp(), entity.getIp(), entity.getPort())) {
    ...
    public Result<Long> apiDeleteRule(@PathVariable("id") Long id) {
        ...
        if (!publishRulesToNacos(oldEntity.getApp(), oldEntity.getIp(), oldEntity.getPort())) {
            logger.error("Publish authority rules failed after rule delete");
        }
        return Result.ofSuccess(id);
    }
    
    private boolean publishRulesToNacos(String app, String ip, Integer port) {
        List<AuthorityRuleEntity> rules = repository.findAllByMachine(
                MachineInfo.of(app, ip, port));
        try {
            publisher.publish(app, rules);
            return true;
        } catch (Exception e) {
            logger.error("Error when publishing auth rules to Nacos", e);
            return false;
        }
    }
     ...
     * @throws Exception
     */
    private List<AuthorityRuleEntity> getNacosRules(String app,
                                                    String ip,
                                                    Integer port) throws Exception {
        List<AuthorityRuleEntity> rules = this.getNacosRules(app);
        if (CollectionUtils.isNotEmpty(rules)) {
            rules = rules.stream()
                            .filter(r -> r.getIp().equals(ip) && r.getPort().equals(port))
     ...
     * @throws Exception
     */
    private List<AuthorityRuleEntity> getNacosRules(String app) throws Exception {
        return provider.getRules(app);
    }
